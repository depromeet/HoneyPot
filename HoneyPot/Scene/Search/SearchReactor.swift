...
final class SearchReactor: Reactor {
        ...
        case .refresh:
            return provider.searchWordService.getWords()
                .flatMap { [weak self] in
                    self?.convertWords(words: $0) ?? .empty()
                }
        case let .inputSearchText(text):
    ...
    func reduce(state: State, mutation: Mutation) -> State {
        ...
        case let .setWords(words):
            state.words = words
        case let .setShouldShowResults(shouldShowResults):
